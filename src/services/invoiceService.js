 ...
 *  - Uses withPage() from puppeteerBrowser.js so pages are always closed
 *    (even on error) and unnecessary resource types are intercepted/aborted
...
function buildInvoiceTemplateData(enrollment = {}) {
    const amountPaid = Number(enrollment.amountPaid || enrollment.amount_paid || 0);
    const couponSavings = Number(enrollment.couponSavings || enrollment.coupon_savings || 0);
    const couponCode = enrollment.couponCode || enrollment.coupon_code || '';
    const hasCoupon = Boolean(couponCode && couponSavings > 0);
    const originalAmount = Number(
        enrollment.originalAmount || enrollment.original_amount || amountPaid + couponSavings || amountPaid
    );
    ...
    const discountTableRow = hasCoupon ? `
        <tr class="discount-row">
            ...
            <td><span class="discount-amount">-${fmt(couponSavings)}</span></td>
        </tr>` : '';
    ...
    const summaryRows = hasCoupon ? `
        <div class="summary-row">
            ...
            <strong style="text-decoration:line-through;color:#9ca3af;">${fmt(originalAmount)}</strong>
        </div>
        ...
        <div class="summary-row net">
            <span>Net Amount</span><strong>${fmt(amountPaid)}</strong>
        </div>` : `
        <div class="summary-row net">
            <span>Net Amount</span><strong>${fmt(amountPaid)}</strong>
        </div>`;
...
export async function buildInvoiceHtml(enrollment) {
    ...
    return replaceTemplateValues(template, buildInvoiceTemplateData(enrollment));
}

export async function generateInvoiceBuffer(enrollment) {
    const html = await buildInvoiceHtml(enrollment);

    // withPage() handles browser singleton + page lifecycle + resource blocking
